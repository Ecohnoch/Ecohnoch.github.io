...
private:
    ...
    string initialPermutation(string);                  // 初始置换，输入64位原始明文，输出64位数据
    string inverseInitialPermutation(string);           // 拟置换，输入L15和R15合成后的新64位，输出密文
    ...
    string feistelCipher(string, string);               // f函数，包含了上述的各种步骤，运用了本类中的各种函数
    ...
    string leftShift(string, int);                      // 循环左移，输入一个二进制串和左移位数
};
...
string DESEncryptor::leftShift(string sourceText, int i){
    string ans;
    while(i--){
        for(int j = 0; j < sourceText.size() - 1; ++j){
            ans[j] = sourceText[j + 1];
        }
        ans[sourceText.size()] = sourceText[0];
    }
    return ans;
}
```
