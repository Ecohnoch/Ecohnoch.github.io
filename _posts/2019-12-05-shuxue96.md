...
那么作者的方案即，对生成器，就定义为$$G(I_{y_r}|y_g)$$，这个生成器在框架里用了两次，一次是从输入图像$$I_{y_r}$$到$$I_{y_g}$$，第二次是将输出再渲染回去$$I_{y_g}->\hat{I}_{y_r}$$。
...
$L_I(G, D_I, I_{y_{y_o}}, y_f) = -\mathbb{E}_{I_{y_o}~\mathbb{P}_o}[D_I(G(I_{y_o}|y_f))] + \mathbb{E}_{I_{y_o}~\mathbb{P}_o}[D_I(I_{y_o})]
...
其中$$A=G_A(I_{y_o}|y_f)$$，lambda同样是惩罚项系数。
...
$L_y(G, D_y, I_{y_o}, y_o, y_f) = \mathbb{E}_{I_{y_o}~\mathbb{P}_o}[\|D_y(I_{y_o}) - y_o\|_2^2] + \mathbb{E}_{I_{y_o}~\mathbb{P}_o}[\|D_y(G(I_{y_o|y_f}))-y_f\|]$

这里原论文里公式书写有错233，也是第一次在这种知名论文里发现小错误。

### Identity loss

$\mathbb{E}_{I_{y_o}~\mathbb{P}_o}[\|G(G(I_{y_o}|y_f)|y_o)-I_{y_o}\|_1]$
