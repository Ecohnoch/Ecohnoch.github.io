...
$ d_{euclidean}(\widetilde{w_i}, \widetilde{w_j}) = \sqrt{{(\widetilde{w_i} - \widetilde{w_j})}^T{(\widetilde{w_i} - \widetilde{w_j})}} =  \sqrt{({\sum}^{-1/2}(w_i - \mu) - {\sum}^{-1/2}(w_j - \mu))^T * ({\sum}^{-1/2}(w_i - \mu) - {\sum}^{-1/2}(w_j - \mu))}$
