...
那么对两个白化后的向量$$\widetilde{w_i}, \widetilde{w_j} $$，计算欧几里得距离：

$ d_{euclidean}(\widetilde{w_i}, \widetilde{w_j}) = \sqrt{{(\widetilde{w_i} - \widetilde{w_j})}^T{(\widetilde{w_i} - \widetilde{w_j})}}  $

$d_{euclidean}(\widetilde{w_i}, \widetilde{w_j}) =  \sqrt{({\sum}^{-1/2}(w_i - \mu) - {\sum}^{-1/2}(w_j - \mu))^T * ({\sum}^{-1/2}(w_i - \mu) - {\sum}^{-1/2}(w_j - \mu))}$

继续推导即为:

$ d_{euclidean}(\widetilde{w_i}, \widetilde{w_j}) = \sqrt{{({w}_i - {w}_j)}^T{({w}_i - {w}_j)}} = d_{mahalanobis}(w_i, w_j)$
